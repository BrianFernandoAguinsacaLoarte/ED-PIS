...
public class UtilVista {
    ...
    public static void setComboParalelo(ComboBoxRect cbxParalelo, int idPralelo) {
        for (int i = 0; i < cbxParalelo.getItemCount(); i++) {
            Paralelo paralelo = (Paralelo) cbxParalelo.getItemAt(i);
            if (paralelo.getId() == idPralelo) {
                cbxParalelo.setSelectedItem(paralelo);
                return;
            }
        }
    }
    ...
    //CARGO PARALELO
    public static void cargarParalelos(ComboBoxRect cbxParalelo) throws VacioExcepcion {
        ParaleloController dc = new ParaleloController();

        cbxParalelo.removeAllItems();
        try {
            for (int i = 0; i < dc.getParalelos().getSize(); i++) {
                cbxParalelo.addItem(dc.getParalelos().get(i));
            }
    ...
    public static Paralelo getComboParalelo(ComboBoxRect cbxRol) {
        return (Paralelo) cbxRol.getSelectedItem();
    }

    //CARGAR ROLES
    public static void cargarRoles(ComboBoxRect cargarRoles) throws VacioExcepcion {
        RolController rc = new RolController();

        cargarRoles.removeAllItems();
        try {
            for (int i = 0; i < rc.getRoles().getSize(); i++) {
                cargarRoles.addItem(rc.getRoles().get(i));
            }
        } catch (VacioExcepcion e) {
            e.printStackTrace();
        }
